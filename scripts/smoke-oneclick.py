...
class Upstream(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if self.path!='/v1/responses' or body.get('model')!='fixture-original-model':
            ...
            with log.open('w') as output:
                ...
                if pathlib.Path(chrome_path).exists():
                    ...
                    with (d/'chrome.log').open('w') as chrome_out:
                        ...
                        job=d/'browser-job.json';job.write_text(json.dumps({'debugPort':debug,'panel':panel,'base':base,'screenshot':str(OUT/'oneclick-panel.png')}))
                        browser=subprocess.run(['node',str(ROOT/'scripts/smoke-oneclick.mjs'),str(job)],text=True,capture_output=True,timeout=45)
                ...
                report.update({'version':'0.4.0-oneclick','startup_did_not_modify_codex':True,'second_launch_reopens':True,'original_config_restored_byte_exact':True,'synthetic_upstream_calls':CALLS,'real_upstream_calls':0,'server_left_running':False})
                (OUT/'oneclick-smoke.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
    finally:
