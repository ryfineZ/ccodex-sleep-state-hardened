// Browser test against the disposable loopback fixture created by smoke-oneclick.py.
...
ws.onmessage=event=>{const msg=JSON.parse(event.data);if(msg.method==='Runtime.exceptionThrown')exceptions.push(msg.params.exceptionDetails.text);if(msg.id){const p=waiting.get(msg.id);waiting.delete(msg.id);if(msg.error)p.reject(new Error(msg.error.message));else p.resolve(msg.result);}};
...
try{
 ...
 const response=await fetch(cfg.base+'/v1/responses',{method:'POST',headers:{'Content-Type':'application/json'},body:'{"model":"fixture-original-model","input":"synthetic browser smoke","stream":true}'});
 ...
 console.log(JSON.stringify({automatic_login:true,one_click_connect:true,request_visible:true,detail_opens:true,restore_and_exit:true,page_exceptions:exceptions.length}));
}finally{ws.close();}
