...
在 [gylive/ccodex-sleep-state](https://github.com/gylive/ccodex-sleep-state) 基础上开发的独立增强版本，保留上游来源说明、版权和 GPL-3.0 许可证。本仓库不是 OpenAI 官方项目，也不代表上游作者为这些修改背书。

## 新增入口

| 程序 | 默认端口 | 用途 |
| --- | --- | --- |
| `ccodex-request-recorder` | `127.0.0.1:17843` | 独立 HTTP/SSE 请求记录、本地查看、模型证据对照、额度观测、离线分析 |
| `ccodex-reliable-proxy` | `127.0.0.1:17842` | 代理熔断与恢复、Cookie 生命周期、客户端 state 保留及流式错误观测 |
| `ccodex-sleep-state` | 原上游入口 | 保留原项目功能；不等同于两个新增程序 |

记录器版本：**`0.3.1-model-override`**。**不提供模型别名配置；强制模型改写是独立可选开关，默认关闭。** 记录会分别保留客户端原始模型、实际出站模型及上游宣告值。模型标识检测不是服务器内部执行身份的证明。

## 从源码启动记录器

需要与 `go.mod` 兼容的 Go 工具链。在仓库根目录运行：
...
终端会输出带本地访问令牌的面板地址。仅启动程序不会看到未接入该端口的 App 流量，不会安装证书、修改系统代理或自动接管 Codex。

**隐私提醒：**默认脱敏模式仍可能保存提示词和回复。不要提交或分享真实抓包、登录凭据、Cookie、运行日志及带令牌的面板地址。运行文件放在已忽略的 `.local/` 中；`full` 记录模式需要额外显式确认。

[记录器说明](README.recorder.md) · [可靠转发说明](README.hardened.md) · [模型开关验证](docs/model-override-validation.md) · [参考项目审查](docs/ccodex-rotate-review.md) · [第三方声明](THIRD_PARTY_NOTICES.md)

## 保留的上游项目说明

以下内容保留自原项目；其中下载链接、联系方式和历史测试结论属于上游版本。两个新增入口请以上面的说明为准。
